...
//---------------------------------------------------- 
  int jugar() { // devuelve 0 (sigue jugando), 1 (ganó) y 2 (perdió). 
    int tiempoJuego;
    ...
    if (!juegoTermino) { //si el juego no termino sigo dibujando el tablero     
       ...
       if (timerSolucion >= 0){ //si apretaron el boton de solucion que me dibuje el camino
            mostrar_camino();
            timerSolucion --;
       }
 ...
 void mousePressed() {
   ...
   if (!juegoTermino && boton_solucion.clickeado() ){
     if (timerSolucion >= 0) // forzamos a que se muestre o no la solucion
      ...
      timerSolucion = 500; //inicializo el contador para mostrar solucion
     }
   } 
 }
